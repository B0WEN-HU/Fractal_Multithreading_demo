...
int main() {
	...
	for (int k = 0; k < K; k++) {
		threads.push_back(std::thread(
			[&](int th_id) {
			std::cout << "Start: " << th_id << std::endl;

			for (int j = 0; j < H; ++j)
				for (int i = th_id; i < W; i += K)
					bmp(j, i) = Mandelbrot(complex{ RMIN + i / RESN, IMIN + j / RESN });

			std::cout << "END:" << th_id << std::endl;
		}, k));
	}
	...
	bmp.save((std::string("Mandelbrot12k_") + std::to_string(THREAD_NUM) + std::string("t.bmp")).data());
	std::cout << "run time: " << duration_cast<duration<double>>(t2 - t1).count() << " seconds.\n";

	std::ofstream fout;
	fout.open("time.log", std::ios::out | std::ios::app);
	fout << "THREAD_NUM:\t" << THREAD_NUM << "\t";
	fout << "run time: " << duration_cast<duration<double>>(t2 - t1).count() << " seconds.\n";
	fout << "**************************************************************************" << std::endl;
	fout.close();
	//getchar();
}
